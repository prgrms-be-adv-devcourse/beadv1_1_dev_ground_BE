...
/**
 * 카테고리 초기 데이터 생성 클래스
 * - 애플리케이션 시작 시 카테고리 데이터가 없으면 자동으로 초기 데이터 생성
 * - test 프로필에서는 실행되지 않음
 */
@Slf4j
@Profile("!test")
...
public class CategoryInit implements ApplicationRunner {
	...
	private final AdminCategoryApplication categoryService;
	...
	private long regist(Long parentId, String name) {
		RegistCategoryDto request = new RegistCategoryDto(name, parentId);
